...
getCompletionAnswer({required question, required model,required context}) async{
  try {
    var headers = {
    ...
    if (response.statusCode == 200) {
      ...
      chatServer.add({'reply': output['choices'][0]['text'],'message': question.toString()});
      if(output['choices'].length>0) {
        print(output['choices']);
        chatList =[];
        for (int i = 0; i <= output['choices'].length; i++) {
          answersList.add({'reply': output['choices'][i]['text']});
          chatList.add({'reply': output['choices'][i]['text'],'message': question.toString()});
...
getChatCompletionAnswer({required question, required model,required context}) async{
  try {
    var headers = {
      'Content-Type': 'application/json',
      'Authorization': 'Bearer $API_Key'
    };
    var response = await http.post(Uri.parse('$BASE_URL/chat/completions'),headers: headers,body: json.encode({
      "model": model.toString(),
    ...
    if (response.statusCode == 200) {
      ...
      chatServer.add({'reply': output['choices'][0]['message']['content'],'message': question.toString()});
      if(output['choices'].length>0) {
        chatList =[];
        for (int i = 0; i <= output['choices'].length; i++) {
          answersList.add({'reply': output['choices'][i]['message']['content']});
          chatList.add({'reply': output['choices'][i]['message']['content'],'message': question.toString()});
// chatServer.add({'reply': chatList[0]['reply'],'message': chatList[0]['message'].toString()});
// print(chatList);
